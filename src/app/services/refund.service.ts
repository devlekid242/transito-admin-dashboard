...
export class RefundService {
	...
	loadRefunds(
		...
		this.http
			.get<RefundListResponse>(`${this.apiBaseUrl}/admin/refunds`, {
				params,
			})
			.pipe(
				catchError((error) => {
					this.error.set(
						"Échec du chargement des demandes de remboursement.",
					);
			...
			.subscribe({
				next: (response) => {
					if (response?.success) {
						...
						this.totalAmountPending.set(
							response.kpis.totalAmountPending,
						);
	...
	loadRefundDetail(id: number) {
		...
		this.http
			.get<{ success: boolean; data: RefundDetail }>(
				`${this.apiBaseUrl}/admin/refunds/${id}`,
			)
			.pipe(
				catchError((error) => {
					this.error.set(
						"Échec du chargement du détail de la demande de remboursement.",
					);
					this.loading.set(false);
					return of(null);
				}),
		...
		this.http
			...
			.pipe(
				catchError((error) => {
					this.error.set(
						error?.error?.message ||
							"Échec du remboursement standard.",
					);
					this.loading.set(false);
					return of(null);
				}),
			...
			.subscribe({
				next: (response) => {
					if (response?.success) {
						...
						// Update KPIs
						this.totalPending.update((count) =>
							Math.max(0, count - 1),
						);
		...
		this.http
			...
			.pipe(
				catchError((error) => {
					this.error.set(
						error?.error?.message ||
							"Échec de la création de la demande de remboursement.",
					);
					this.loading.set(false);
					return of(null);
				}),
	...
	searchClients(query: string) {
		...
		this.http
			.get<LookupResponse>(
				`${this.apiBaseUrl}/admin/refunds/lookup/clients`,
				{
					params: { q: query },
				},
			)
	...
	searchReservations(query: string, userId?: number | null) {
		...
		this.http
			.get<LookupResponse>(
				`${this.apiBaseUrl}/admin/refunds/lookup/reservations`,
				{ params },
			)
			.pipe(catchError(() => of(null)))
			.subscribe((response) => {
	...
	searchAgencies(query: string) {
		...
		this.http
			.get<LookupResponse>(
				`${this.apiBaseUrl}/admin/refunds/lookup/agencies`,
				{
					params: { q: query },
				},
			)
