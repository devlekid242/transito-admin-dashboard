...
@Component({
  ...
  imports: [CommonModule, FormsModule, PageHeaderComponent, StatCardComponent, LineChartComponent, BarChartComponent],
...
export class FinancialStatsPage {
  ...
  readonly loading = computed(() => this.financialService.loadingFinancialStats());
  readonly error = computed(() => this.financialService.errorFinancialStats());
  ...
  readonly caChartLabels = computed(() => this.data()?.chartData.labels ?? []);
  ...
  readonly benefitVsCommissionsLabels = computed(() => this.data()?.chartData.labels ?? []);
  ...
  readonly financialDetailByAgency = computed(() => this.data()?.financialDetailByAgency ?? []);

  readonly walletBalances = computed(() => this.data()?.walletBalances ?? { total: 0, available: 0, reserved: 0 });

  readonly withdrawals = computed(() => this.data()?.withdrawals ?? { totalCount: 0, totalAmount: 0, pendingCount: 0, pendingAmount: 0 });

  readonly refunds = computed(() => this.data()?.refunds ?? { totalCount: 0, totalAmount: 0, pendingCount: 0, pendingAmount: 0 });
