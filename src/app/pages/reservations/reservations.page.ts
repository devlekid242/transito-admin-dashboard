...
export class ReservationsPage implements OnInit {
	...
	readonly filteredReservations =
		this.reservationService.filteredReservations;
	...
	readonly seatIndexes = computed(() =>
		Array.from({ length: this.seatCount() }, (_, i) => i),
	);
	...
	readonly searchPlaceholder =
		"Rechercher par réf, passager, agence, trajet...";
	...
	readonly paymentOptions: {
		...
		{
			value: "new_payment",
			label: "Initier un nouveau paiement",
			description: "Envoyer une nouvelle demande de paiement au client",
		},
		{
			value: "link_existing",
			label: "Lier à un paiement existant",
			description: "Lier à un paiement non lié existant",
		},
	];
	...
	private loadAgencies(): void {
		...
		this.agencyService.getAgencies().subscribe((response) => {
			if (response.success && "data" in response && response.data) {
				const agencies: SearchSelectItem[] = response.data.map((a) => ({
					id: String(a.id),
					label: a.name,
	...
	getStatusLabel(status: string): string {
		return this.reservationService.getStatusLabel(
			...
			next: (response) => {
				if (response.success && response.data) {
					...
					if (
						...
						reservation.paymentLogs.length > 0
					) {
	...
	onUserSelected(item: SearchSelectItem | null) {
		...
		if (item && this.paymentOption() === "link_existing") {
			this.reservationService
				.getUnlinkedPaymentsForUser(Number(item.id))
				.subscribe();
		} else {
			this.selectedPayment.set(null);
		}
	...
	onAgencySelected(item: SearchSelectItem | null) {
		...
		if (item) {
			this.reservationService
				.getTripsByAgency(Number(item.id))
				.subscribe();
		} else {
	...
	onPaymentOptionChange(option: PaymentOption) {
		...
		if (option === "link_existing" && this.selectedUser()) {
			this.reservationService
				.getUnlinkedPaymentsForUser(Number(this.selectedUser()!.id))
				.subscribe();
		} else {
			this.selectedPayment.set(null);
		}
	...
	onPaymentSelected(item: SearchSelectItem | null) {
		...
		if (item && this.paymentOption() === "link_existing") {
			...
			const payment = this.paymentItems().find(
	...
	submitReservation() {
		...
		if (this.seatCount() > 0) {
			...
			if (hasNullSeat) {
				this.formError.set(
					"Veuillez indiquer un numéro de siège pour chaque place.",
				);
				this.isLoadingForm.set(false);
				return;
			}
		...
		if (
			this.paymentOption() === "link_existing" &&
			!this.selectedPayment()
		) {
			this.formError.set(
				"Veuillez sélectionner un paiement existant à lier.",
			);
			this.isLoadingForm.set(false);
			return;
		}
		...
		if (this.formMode() === "edit" && reservationId) {
			...
			this.reservationService
				.updateReservation(reservationId, {
					...
					paymentStatus:
						dto.paymentOption === "link_existing"
							? "PAID"
							: "PENDING",
				})
				.subscribe({
	...
	onTripSearch(query: string) {
		if (query.length >= 2 && this.selectedAgency()) {
			this.reservationService
				.getTripsByAgency(Number(this.selectedAgency()!.id), query)
				.subscribe();
		}
	}
	...
	onAgencySearch(query: string) {
		if (query.length >= 2) {
			...
			const filtered = this.agencyItems().filter(
				(a) =>
	...
	getReservationDate(reservation: Reservation): string {
		return reservation.trip?.date
			? this.reservationService.formatDate(reservation.trip.date)
			: "N/A";
	}
	...
	getReservationStatusBadgeVariant(
		...
		return this.reservationService.getStatusBadgeVariant(
			reservation.status,
		);
	}
