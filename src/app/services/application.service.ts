...
export type ApplicationStatus = 'PENDING' | 'UNDER_REVIEW' | 'APPROVED' | 'REJECTED' | 'ALL';

// Document Type Types
export type DocumentType = 'RCCM' | 'NINEA' | 'ASSURANCE' | 'CARTE_GRISE' | 'CONTRAT' | 'AUTRE';
...
export class ApplicationService {
  ...
  readonly documentTypeOptions = signal<{ value: DocumentType; label: string; icon: string }[]>([]);
  ...
  readonly filteredApplications = computed(() => {
    ...
    return applications.filter(application => {
      const matchesSearch = search === '' ||
        ...
        (application.agencyName?.toLowerCase().includes(search) ?? false) ||
        (application.legalRepresentative?.toLowerCase().includes(search) ?? false) ||
        (application.email?.toLowerCase().includes(search) ?? false) ||
        (application.phone?.toLowerCase().includes(search) ?? false);

      const matchesStatus = status === 'ALL' || application.status === status;
      const matchesCity = city === '' || (application.city?.toLowerCase().includes(city) ?? false);
  ...
  getApplications(
    ...
    return this.http
      .get<ApiResponse<Application[]>>(`${this.apiBaseUrl}/admin/applications`, { params })
      .pipe(
        tap(response => {
          if (response.success && response.data) {
            ...
            if (response.pagination) {
              ...
              this.totalApplications.set(response.pagination.total);
            }
          }
        }),
        catchError(error => {
          console.error('Error fetching applications:', error);
          return of({ success: false, message: error.error?.message || 'Erreur lors de la récupération des candidatures' });
        }),
  ...
  getApplicationKpis(startDate?: string, endDate?: string) {
    ...
    return this.http
      .get<ApiResponse<ApplicationKpis>>(`${this.apiBaseUrl}/admin/applications/kpis`, { params })
      .pipe(
        tap(response => {
          if (response.success && response.data) {
  ...
  getApplicationDetail(id: number) {
    ...
    return this.http
      .get<ApiResponse<ApplicationDetail>>(`${this.apiBaseUrl}/admin/applications/${id}`)
      .pipe(
        tap(response => {
          if (response.success && response.data) {
        ...
        catchError(error => {
          console.error(`Error fetching application detail ${id}:`, error);
          return of({ success: false, message: error.error?.message || 'Erreur lors de la récupération de la candidature' });
        }),
  ...
  approveApplication(id: number, data: ApproveApplicationDto) {
    ...
    return this.http
      .post<ApiResponse<any>>(`${this.apiBaseUrl}/admin/applications/${id}/approve`, data)
      .pipe(
        tap(response => {
          if (response.success) {
            this.actionSuccess.set('Candidature approuvée avec succès');
            // Refresh the application detail
            this.getApplicationDetail(id).subscribe();
            // Refresh the list
            this.refreshApplications();
          } else {
            this.actionError.set(response.message || 'Erreur lors de l\'approbation');
          }
        }),
        catchError(error => {
          console.error(`Error approving application ${id}:`, error);
          this.actionError.set(error.error?.message || 'Erreur lors de l\'approbation');
          return of({ success: false, message: error.error?.message || 'Erreur lors de l\'approbation' });
        }),
  ...
  rejectApplication(id: number, data: RejectApplicationDto) {
    ...
    return this.http
      .post<ApiResponse<any>>(`${this.apiBaseUrl}/admin/applications/${id}/reject`, data)
      .pipe(
        tap(response => {
          if (response.success) {
            this.actionSuccess.set('Candidature rejetée avec succès');
            // Refresh the application detail
            this.getApplicationDetail(id).subscribe();
            // Refresh the list
            this.refreshApplications();
          } else {
            this.actionError.set(response.message || 'Erreur lors du rejet');
          }
        }),
        catchError(error => {
          console.error(`Error rejecting application ${id}:`, error);
          this.actionError.set(error.error?.message || 'Erreur lors du rejet');
  ...
  startReview(id: number) {
    ...
    return this.http
      .post<ApiResponse<any>>(`${this.apiBaseUrl}/admin/applications/${id}/start-review`, {})
      .pipe(
        tap(response => {
          if (response.success) {
        ...
        catchError(error => {
          console.error(`Error starting review for application ${id}:`, error);
          this.actionError.set(error.error?.message || 'Erreur lors de la mise en revue');
          return of({ success: false, message: error.error?.message || 'Erreur lors de la mise en revue' });
        }),
        tap(() => this.loadingAction.set(false))
      );
  }
  ...
  getDocumentTypeOptions() {
    ...
    return this.http
      .get<ApiResponse<{ value: string; label: string }[]>>(`${this.apiBaseUrl}/admin/applications/document-types`)
      .pipe(
        tap(response => {
          if (response.success && response.data) {
            const options = response.data.map(opt => ({
              ...
              label: opt.label,
              icon: this.getDocumentTypeIcon(opt.value as DocumentType),
            }));
        ...
        catchError(error => {
          ...
          const defaultOptions = [
            { value: 'RCCM', label: 'Registre du Commerce et du Crédit Mobilier', icon: 'fa-file-contract' },
            { value: 'NINEA', label: 'Numéro d\'Identification Nationale des Employeurs', icon: 'fa-file-lines' },
            { value: 'ASSURANCE', label: 'Assurance Flotte', icon: 'fa-shield-halved' },
            { value: 'CARTE_GRISE', label: 'Carte Grise', icon: 'fa-car' },
            { value: 'CONTRAT', label: 'Contrat Social', icon: 'fa-file-signature' },
            { value: 'AUTRE', label: 'Autre Document', icon: 'fa-file' },
          ];
          this.documentTypeOptions.set(defaultOptions);
          return of({ success: true, data: defaultOptions });
        })
      );
  }
  ...
  getMissingDocumentTypes(application: ApplicationDetail): DocumentType[] {
    ...
    return requiredTypes.filter(type => !existingTypes.includes(type)) as DocumentType[];
  }
