...
export class TripService {
	...
	readonly filteredTrips = computed(() => {
		...
		return trips.filter((trip) => {
			...
			const matchesAgency =
				agencyId === null || trip.agencyId === agencyId;
	 ...
	 */
	getTrips(page: number = 1, limit: number = 10, filters: TripFilters = {}) {
		...
		return this.http
			.get<
				ApiResponse<Trip[]>
			>(`${this.apiBaseUrl}/admin/trips`, { params })
			.pipe(
				tap((response) => {
	...
	getTripKpis(startDate?: string, endDate?: string, agencyId?: number) {
		...
		return this.http
			.get<
				ApiResponse<TripKpis>
			>(`${this.apiBaseUrl}/admin/trips/kpis`, { params })
			.pipe(
				tap((response) => {
					if (response.success && response.data) {
	...
	getTripDetail(id: number) {
		...
		return this.http
			...
			>(`${this.apiBaseUrl}/admin/trips/${id}`)
			.pipe(
				tap((response) => {
					if (response.success && response.data) {
	...
	getTripManifest(tripId: number) {
		...
		return this.http
			...
			.pipe(
				...
				catchError((error) => {
					console.error(
						`Error fetching manifest for trip ${tripId}:`,
						error,
					);
					return of({
						success: false,
						message:
							error.error?.message ||
							"Erreur lors de la récupération du manifeste",
					});
				}),
	...
	): "approved" | "pending" | "rejected" | "missing" | "verified" | "info" {
		switch (status) {
			case "SCHEDULED":
				return "pending";
			case "IN_PROGRESS":
	...
	getPassengerStatusLabel(
		status: "BOARDING" | "BOARDED" | "NO_SHOW" | string,
	): string {
		switch (status) {
			case "BOARDED":
				return "Embarqué";
	 ...
	 */
	getPassengerStatusBadgeVariant(
		status: "BOARDING" | "BOARDED" | "NO_SHOW" | string,
	): "approved" | "pending" | "rejected" | "verified" | "info" {
		switch (status) {
			case "BOARDED":
				return "verified";
			case "BOARDING":
