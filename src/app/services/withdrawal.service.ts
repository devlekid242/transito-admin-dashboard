...
export type ApproveResultCallback = (result: {
	success: boolean;
	message?: string;
}) => void;
...
export class WithdrawalService {
	...
	loadWithdrawals(
		...
		this.http
			.get<WithdrawalListResponse>(
				`${this.apiBaseUrl}/admin/withdrawals`,
				{ params },
			)
			.pipe(
				catchError((error) => {
					this.error.set(
						"Échec du chargement des demandes de retrait.",
					);
				...
				next: (response) => {
					if (response?.success) {
						this.withdrawals.set(
							response.data.map(normalizeWithdrawal),
						);
	...
	loadWithdrawal(id: number) {
		...
		this.http
			.get<{ success: boolean; data: Withdrawal }>(
				`${this.apiBaseUrl}/admin/withdrawals/${id}`,
			)
			.pipe(
				catchError((error) => {
	...
	checkSolvency(id: number) {
		...
		this.http
			.get<SolvencyCheckResponse>(
				`${this.apiBaseUrl}/admin/withdrawals/${id}/check-solvency`,
			)
			.pipe(
				catchError((error) => {
					this.error.set(
						error?.error?.message ||
							"Échec de la vérification de solvabilité.",
					);
					this.loading.set(false);
					return of(null);
				}),
	...
	approveWithdrawalWithSolvency(
		...
		this.http
			...
			.pipe(
				catchError((error) => {
					const message =
						error?.error?.message || "Échec de l'approbation.";
			...
			.subscribe({
				next: (response) => {
					if (response?.success) {
							...
							list.map((w) =>
								w.id === id
									? {
											...
											processedByAdminId:
												response.processedByAdminId,
											processedByAdminName:
												response.processedByAdminName,
										}
									: w,
							),
						);
						...
						const message =
							response.message || "Risque financier détecté.";
						this.error.set(message);
						onResult?.({ success: false, message });
					}
					this.loading.set(false);
