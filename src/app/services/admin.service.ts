...
export type AdminRoleType = 'SUPER_ADMIN' | 'FINANCE_ADMIN' | 'MODERATION_ADMIN' | 'SUPPORT_ADMIN';
...
export class AdminService {
  ...
  getAdmins(page: number = 1, limit: number = 10) {
    ...
    return this.http
      .get<AdminListResponse>(`${this.apiBaseUrl}/admin/admins`, { params })
      .pipe(
        tap(response => {
          if (response.success && response.data) {
            this.admins.set(response.data);
            this.currentPage.set(page);
            this.totalAdmins.set(response.pagination?.total ?? response.data.length);
            this.totalPages.set(response.pagination?.totalPages ?? 1);
          }
        }),
        catchError(error => {
          console.error('Error fetching admins:', error);
          this.lastError.set(error?.error?.message ?? 'Erreur lors du chargement des administrateurs');
          return of({ success: false, data: [] } as AdminListResponse);
        }),
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError(error => {
          ...
          this.lastError.set(error?.error?.message ?? 'Erreur lors du chargement de l\'administrateur');
          return of({ success: false, data: null } as AdminDetailResponse);
        }),
  ...
  createAdmin(payload: CreateAdminRequest) {
    ...
    return this.http
      .post<AdminDetailResponse>(`${this.apiBaseUrl}/admin/admins`, payload)
      .pipe(
        tap(response => {
        ...
        catchError(error => {
          ...
          this.lastError.set(error?.error?.message ?? 'Erreur lors de la création de l\'administrateur');
          return of({ success: false, data: null } as AdminDetailResponse);
        }),
  ...
  updateAdmin(id: number, payload: UpdateAdminRequest) {
    ...
    return this.http
      .put<AdminDetailResponse>(`${this.apiBaseUrl}/admin/admins/${id}`, payload)
      .pipe(
        tap(response => {
          const updatedAdmin = response.data;
          if (response.success && updatedAdmin) {
            this.admins.update(list =>
              list.map(admin => (admin.id === id ? updatedAdmin : admin)),
            );
        ...
        catchError(error => {
          ...
          this.lastError.set(error?.error?.message ?? 'Erreur lors de la mise à jour de l\'administrateur');
          return of({ success: false, data: null } as AdminDetailResponse);
        }),
        tap(() => this.saving.set(false)),
      );
  ...
  deleteAdmin(id: number) {
    ...
    return this.http
      .delete<AdminDetailResponse>(`${this.apiBaseUrl}/admin/admins/${id}`)
      .pipe(
        tap(response => {
          if (response.success) {
            this.admins.update(list => list.filter(admin => admin.id !== id));
            this.totalAdmins.update(count => Math.max(0, count - 1));
          }
        }),
        catchError(error => {
          console.error(`Error deleting admin ${id}:`, error);
          this.lastError.set(error?.error?.message ?? 'Erreur lors de la suppression de l\'administrateur');
          return of({ success: false, data: null } as AdminDetailResponse);
        }),
