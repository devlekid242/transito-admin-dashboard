...
export class WalletService {
   ...
   */
  loadWallets(page: number = 1, perPage: number = 20, filters: FilterOptions = {}) {
    ...
    this.http
      .get<WalletListResponse>(`${this.apiBaseUrl}/admin/wallets`, { params })
      .pipe(
        catchError((error) => {
  ...
  loadWalletDetail(id: number) {
    ...
    this.http
      .get<{ success: boolean; data: WalletDetailResponse }>(`${this.apiBaseUrl}/admin/wallets/${id}`)
      .pipe(
        catchError((error) => {
          this.error.set('Échec du chargement du détail du portefeuille.');
          this.loading.set(false);
          return of(null);
        }),
      )
        ...
        next: (response) => {
          if (response?.success) {
            ...
            this.wallets.update((list) =>
              list.map((w) =>
                w.id === id ? { ...w, available: response.newBalance || w.available } : w,
              ),
            );
            ...
            if (response.newBalance !== undefined) {
              this.totalAvailable.update((total) => {
                const oldWallet = this.wallets().find((w) => w.id === id);
                if (oldWallet) {
                  return total - oldWallet.available + response.newBalance!;
                }
                return total;
              });
            }
  ...
  freezeWallet(id: number, reason?: string) {
    ...
    this.http
      ...
      .pipe(
        catchError((error) => {
          this.error.set(error?.error?.message || 'Échec du gel du portefeuille.');
          this.loading.set(false);
          return of(null);
        }),
      )
              ...
              list.map((w) =>
                w.id === id 
                  ? {
                      ...
                      frozenByAdminId: response.frozenByAdminId,
                      frozenByAdminName: response.frozenByAdminName,
                    }
                  : w,
              ),
            );
  ...
  unfreezeWallet(id: number, reason?: string) {
    ...
    this.http
      ...
      .pipe(
        catchError((error) => {
          this.error.set(error?.error?.message || 'Échec du dégel du portefeuille.');
          this.loading.set(false);
          return of(null);
        }),
      )
  ...
  getWalletSummary(id: number) {
    ...
    this.http
      ...
      .pipe(
        catchError((error) => {
          this.error.set('Échec du chargement du résumé du portefeuille.');
          this.loading.set(false);
          return of(null);
        }),
      )
