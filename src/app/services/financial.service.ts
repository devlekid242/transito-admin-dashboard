...
export class FinancialService {
	...
	loadRevenueAnalysis(filter?: DateFilter) {
		...
		this.http
			.get<RevenueAnalysisResponse>(
				`${this.apiBaseUrl}/admin/financial/revenue-analysis`,
				{ params },
			)
			.pipe(
				catchError((error) => {
					this.errorRevenueAnalysis.set(
						"Échec du chargement de l'analyse des revenus.",
					);
	...
	loadFinancialStats(filter?: DateFilter) {
		...
		this.http
			.get<FinancialStatsResponse>(
				`${this.apiBaseUrl}/admin/financial/stats`,
				{ params },
			)
			.pipe(
				catchError((error) => {
					this.errorFinancialStats.set(
						"Échec du chargement des statistiques financières.",
					);
	...
	loadTransactionHistory(filter?: TransactionFilter) {
		...
		this.http
			.get<TransactionHistoryResponse>(
				`${this.apiBaseUrl}/admin/financial/transactions`,
				{ params },
			)
			.pipe(
				catchError((error) => {
					this.errorTransactionHistory.set(
						"Échec du chargement de l'historique des transactions.",
					);
	 ...
	 */
	getFinancialStatsAgencyBarData(): {
		label: string;
		value: number;
		color: string;
	}[] {
