...
export class AdminProfileService {
	...
	updateProfile(payload: UpdateProfileRequest) {
		...
		return this.http
			.put<{
				...
				message: string;
				data: AdminProfile;
			}>(`${this.apiBaseUrl}/admin/profile/me`, payload)
			.pipe(
				tap((response) => {
					if (response.success) {
	...
	changePassword(payload: ChangePasswordRequest) {
		...
		return this.http
			.put<{
				success: boolean;
				message: string;
			}>(`${this.apiBaseUrl}/admin/profile/me/password`, payload)
			.pipe(
				tap((response) => {
					if (response.success) {
						this.successMessage.set(response.message);
	...
	uploadPhoto(photoUrl: string) {
		...
		return this.http
			...
			}>(`${this.apiBaseUrl}/admin/profile/me/photo`, { photoUrl })
			.pipe(
				tap((response) => {
	...
	uploadPhotoFile(file: File) {
		...
		return this.http
			.post<{
				...
				message: string;
				data: { profilePhotoUrl: string };
			}>(`${this.apiBaseUrl}/admin/profile/me/photo`, formData, {
				reportProgress: true,
				observe: "events",
			})
			.pipe(
				tap((event: HttpEvent<any>) => {
					if (event.type === HttpEventType.UploadProgress) {
						...
						if (
							event.body?.success &&
							event.body.data?.profilePhotoUrl
						) {
				...
				catchError((error) => {
					...
					this.error.set(
						error?.error?.message ??
							"Erreur lors du téléchargement du fichier photo",
					);
	...
	loadActivityLogs(page: number = 1, limit: number = 20) {
		...
		return this.http
			.get<ActivityLogResponse>(
				`${this.apiBaseUrl}/admin/profile/activity`,
				{
					params,
				},
			)
