...
export type ReservationStatus = 'PENDING' | 'CONFIRMED' | 'COMPLETED' | 'CANCELLED' | 'NO_SHOW' | 'FAILED' | 'REFUNDED';
export type ReservationPaymentStatus = 'PENDING' | 'PAID' | 'CANCELLED' | 'FAILED' | 'REFUNDED';
...
export class ReservationService {
  ...
  readonly filteredReservations = computed(() => {
    ...
    return reservations.filter(reservation => {
      const matchesSearch = search === '' ||
        ...
        (reservation.user?.fullName?.toLowerCase().includes(search) ?? false) ||
        (reservation.user?.phoneNumber?.toLowerCase().includes(search) ?? false) ||
        (reservation.trip?.route?.toLowerCase().includes(search) ?? false) ||
        (reservation.agency?.name?.toLowerCase().includes(search) ?? false);

      const matchesStatus = status === 'ALL' || reservation.status === status;
      const matchesAgency = agencyId === null || reservation.agency?.id === agencyId;
  ...
  getReservations(
    ...
    return this.http
      .get<ApiResponse<Reservation[]>>(`${this.apiBaseUrl}/admin/reservations`, { params })
      .pipe(
        tap(response => {
          if (response.success && response.data) {
            ...
            if (response.pagination) {
              ...
              this.totalReservations.set(response.pagination.total);
            }
          }
        }),
        catchError(error => {
          ...
          return of<ApiResponse<Reservation[]>>({
            success: false,
            message: error.error?.message || 'Erreur lors de la récupération des réservations',
          });
        }),
  ...
  getReservationKpis(startDate?: string, endDate?: string) {
    ...
    return this.http
      .get<ApiResponse<ReservationKpis>>(`${this.apiBaseUrl}/admin/reservations/kpis`, { params })
      .pipe(
        tap(response => {
          if (response.success && response.data) {
        ...
        catchError(error => {
          ...
          return of<ApiResponse<ReservationKpis>>({
            success: false,
            message: error.error?.message || 'Erreur lors de la récupération des KPIs',
          });
        }),
  ...
  getReservationDetail(id: number) {
    ...
    return this.http
      .get<ApiResponse<ReservationDetail>>(`${this.apiBaseUrl}/admin/reservations/${id}`)
      .pipe(
        tap(response => {
          if (response.success && response.data) {
        ...
        catchError(error => {
          console.error(`Error fetching reservation detail ${id}:`, error);
          return of<ApiResponse<ReservationDetail>>({
            success: false,
            message: error.error?.message || 'Erreur lors de la récupération de la réservation',
          });
        }),
  ...
  createReservation(data: CreateReservationDto) {
    return this.http
      .post<ApiResponse<ReservationDetail>>(`${this.apiBaseUrl}/admin/reservations`, data)
      .pipe(
        tap(response => {
          if (response.success && response.data) {
        ...
        catchError(error => {
          ...
          return of<ApiResponse<ReservationDetail>>({
            success: false,
            message: error.error?.message || 'Erreur lors de la création de la réservation',
          });
        })
      );
  }
  ...
  updateReservation(id: number, data: UpdateReservationDto) {
    return this.http
      .put<ApiResponse<ReservationDetail>>(`${this.apiBaseUrl}/admin/reservations/${id}`, data)
      .pipe(
        tap(response => {
          if (response.success && response.data) {
        ...
        catchError(error => {
          ...
          return of<ApiResponse<ReservationDetail>>({
            success: false,
            message: error.error?.message || 'Erreur lors de la mise à jour de la réservation',
          });
        })
      );
  }
  ...
  cancelReservation(id: number, data: CancelReservationDto = {}) {
    return this.http
      .put<ApiResponse<ReservationDetail>>(`${this.apiBaseUrl}/admin/reservations/${id}/cancel`, data)
      .pipe(
        tap(response => {
          if (response.success && response.data) {
            // Update current reservation if it's the one we're viewing
        ...
        catchError(error => {
          ...
          return of<ApiResponse<ReservationDetail>>({
            success: false,
            message: error.error?.message || 'Erreur lors de l\'annulation de la réservation',
          });
        })
      );
  }
   ...
   */
  getTripsByAgency(agencyId: number | null = null, search: string = '', limit: number = 20) {
    ...
    return this.http
      .get<ApiResponse<TripSearchItem[]>>(`${this.apiBaseUrl}/admin/reservations/trips`, { params })
      .pipe(
        tap(response => {
          if (response.success && response.data) {
        ...
        catchError(error => {
          ...
          return of<ApiResponse<TripSearchItem[]>>({ success: false, data: [] });
        }),
  ...
  getUsersForSearch(search: string = '', limit: number = 20) {
    ...
    return this.http
      .get<ApiResponse<UserSearchItem[]>>(`${this.apiBaseUrl}/admin/reservations/users`, { params })
      .pipe(
        tap(response => {
          if (response.success && response.data) {
        ...
        catchError(error => {
          ...
          return of<ApiResponse<UserSearchItem[]>>({ success: false, data: [] });
        }),
  ...
  getUnlinkedPaymentsForUser(userId: number) {
    return this.http
      .get<ApiResponse<PaymentSearchItem[]>>(`${this.apiBaseUrl}/admin/reservations/users/${userId}/unlinked-payments`)
      .pipe(
        tap(response => {
          if (response.success && response.data) {
        ...
        catchError(error => {
          console.error(`Error fetching unlinked payments for user ${userId}:`, error);
          return of<ApiResponse<PaymentSearchItem[]>>({ success: false, data: [] });
        })
      );
  }
  ...
  getTripDetails(tripId: number) {
    return this.http
      .get<ApiResponse<TripDetail>>(`${this.apiBaseUrl}/admin/reservations/trips/${tripId}`)
      .pipe(
        catchError(error => {
          console.error(`Error fetching trip details ${tripId}:`, error);
          return of<ApiResponse<TripDetail>>({ success: false, data: undefined });
        })
      );
  }
   ...
   */
  getStatusBadgeVariant(status: ReservationStatus | string): 'approved' | 'pending' | 'rejected' | 'missing' | 'verified' {
    switch (status) {
      case 'CONFIRMED':
      case 'PAID':
        return 'approved';
   ...
   */
  getTicketStatusBadgeVariant(status: TicketStatus | string): 'approved' | 'pending' | 'rejected' {
    switch (status) {
      case 'BOARDED': return 'approved';
