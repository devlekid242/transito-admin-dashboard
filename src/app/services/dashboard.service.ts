...
export class DashboardService {
	...
	readonly reservationsChartData = signal<{
		labels: string[];
		series: number[];
	} | null>(null);
	readonly newUsersChartData = signal<{
		labels: string[];
		series: number[];
	} | null>(null);
	...
	loadKpis() {
		...
		this.http
			.get<{ success: boolean; data: DashboardKpis }>(
	...
	loadActivity() {
		...
		this.http
			.get<{ success: boolean; data: ActivityItem[] }>(
				`${this.apiBaseUrl}/admin/dashboard/activity`,
			)
			.pipe(
				catchError((error) => {
					this.error.set(
						"Échec du chargement de l'activité récente.",
					);
	...
	loadAlerts() {
		...
		this.http
			.get<{ success: boolean; data: Alert[] }>(
				`${this.apiBaseUrl}/admin/dashboard/alerts`,
			)
			.pipe(
				catchError((error) => {
				...
				next: (response) => {
					if (response?.success && response.data) {
						this.revenueChartData.set({
							...
							series: [
								response.data.revenueSeries,
								response.data.newUsersSeries,
							],
						});
					}
				},
